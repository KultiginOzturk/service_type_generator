...
def resolve_final_signals(api_signals, word_signals):
    ...
    api_has_reservice = api_signals.get("has_reservice", api_signals["isRecurring"])
    ...
    if word_signals["reservice"] is None:
        ...
        conflicts["reservice_reason"] = f"Conflict: API says reservice={api_reservice}, word signals={word_signals['reservice']}"
    ...
    if word_signals["recurring"] is None:
        ...
        conflicts["recurring_reason"] = f"Conflict: API recurring={api_recurring}, word recurring={word_signals['recurring']}"
    ...
    if word_signals["zero_time"] is None:
        ...
        conflicts["zero_time_reason"] = f"Conflict: API zeroTime={api_zero_time}, word zeroTime={word_signals['zero_time']}"
    ...
    if word_signals["has_reservice"] is None:
        ...
        conflicts["has_reservice_reason"] = f"Conflict: API hasReservice={api_has_reservice}, word hasReservice={word_signals['has_reservice']}"
    
    # Carry API has_reservice forward regardless of word override state for constraints.
    # final_signals["has_reservice"] already set above.
...
def check_business_constraints(final_signals):
    ...
    # 1.0 - if recurring=True then has_reservice=True
    ...
    # 1.1 - anything that is isRervice = True cannot have has_reservice as True, set has_reservice to False
...
def analyze_service_type(row, appointments_df, subscriptions_df, service_types_df, now, client_id):
    ...
    logger.info(f"Analysis complete for TYPE_ID {type_id}: AskClient={askclient}, Conflicts={has_conflicts}, Constraint violations={len(constraint_violations)}")
